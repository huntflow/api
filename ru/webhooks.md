...
На каждый созданных вебхук Хантфлоу будет предпринимать до 4-х попыток отправить событие с интервалами 0, 10, 100, 1000 секунд. 

Сервис принимающий наши события должен корректно отвечать кодом `200` или `204`, информируя о том что событие было получено. 

Мы отправляем событие и ожидаем, что оно корректно получено, иначе будут предприняты попытки его переотправить, а по истечении количества попыток, неработающий вебхук будет автоматически отключен. При отключении вебхука всем управляющим в организации будет отправлено об этом письмо.
