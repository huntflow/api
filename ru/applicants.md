...
* `pd_agreement_state` — состояние согласия на хранение Персональных Данных. Доступен, если включен модуль Персональных Данных. Если передается, то вернутся только кандидаты, которые имеют переданное состояние согласия на хранение Персональных Данных. Возможные значения: `not_sent`, `sent`, `accepted`, `declined`.
